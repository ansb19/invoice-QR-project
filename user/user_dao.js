...
export class USER_DAO { // 유저 테이블 관련 데이터 베이스 처리 하는 곳
  ...
  async find_id(email) {
    try {
      ...
      console.error("database query [find_id] is problem: " + err.stack);
      throw err;
    }
  }
  ...
  async find_password(id, email) {
    try {
      ...
      console.error("database query [find_password] is problem: " + err.stack);
      throw err;
    }
  }
  ...
  async change_password(id, password) {
    try {
      ...
      console.error("database query [change_password] is problem: " + err.stack);
      throw err;
    }
  }
  ...
  async user_access_change(grade_status, id) {
    try {
      ...
      console.error("database query [user_access_change] is problem: " + err.stack);
      throw err;
    }
  }
  ...
  async user_select() {
    try {
      ...
      console.error("database query [user_select] is problem: " + err.stack);
      throw err;
    }
  }
  ...
  async user_search(keyword) {
    try {
      ...
      console.error("database query [user_search] is problem: " + err.stack);
      throw err;
    }
  }
  ...
  async email_cert_create(id, email_cert_number) {
    try {
      ...
      console.error("database query [email_cert_create] is problem: " + err.stack);
      throw err;
    }
  }
  ...
  async email_cert_read(id) {
    try {
      ...
      console.error("database query [email_cert_read] is problem: " + err.stack);
      throw err;
    }
  }
  ...
  async change_verified_user(id) {
    try {
      ...
      const result = await this.transaction([sql], values);
      console.log(
        `${id}의 이메일 인증완료.. Number of records insert: ${result.affectedRows}`
      );
    }
    catch (err) {
      console.error("database query [email_cert_read] is problem: " + err.stack);
  ...
  async email_cert_delete(id) {
    ...
    console.error("database query [email_cert_delete] is problem: " + err.stack);
    throw err;
  }
  }
  ...
  async check_duplicate_id(id) {
    try {
      ...
      let values = [id];
      const [result] = await this.transaction([sql], values);
      return result; //0 이면 없음 1이면 있음
    }
    catch (err) {
      console.error("database query [check_duplicate_id] is problem: " + err.stack);
      throw err;
    }
  }
  ...
  async find_name(id){
    try{
      ...
      console.error("database query [find_name] is problem: " + err.stack);
      throw err;
    }
  }
}
